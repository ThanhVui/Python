...
def load_and_print_summary(file_name):
    with open(file_name, mode='r', newline='', encoding='utf-8') as json_file:
        ...
        print(f"Summary Of Max Temperature: {sum_temperature_2m_maxs}\u00b0C")
        print(f"Summary Of Min Temperature: {sum_temperature_2m_mins}\u00b0C")
...
def find_top3_warmest_days(data):
    ...
    list_top3_warmest_days = []
    list_all_warmest_days = []
    
    for index in range(len(dates)):
        date = dates[index]
        max_temp = temperature_2m_maxs[index]
        
        list_all_warmest_days.append((date, max_temp))
        
    list_top3_warmest_days = sorted(list_all_warmest_days, key=lambda x : x[1], reverse=True)[:3]

    return list_top3_warmest_days
...
def detect_stable_days(data):
    ...
    for index in range(len(dates)):
        ...
        stable_day = max_temp - min_temp
        
        if stable_day <= 5:
            list_stable_days.append((date, float(f"{stable_day:.2f}")))
...
def export_stable_day_to_csv(data, list_stable_days):
    ...
    file_name_csv = r"D:\Study-AI\Python\Practical\9_PRP201c_SP25_PE1_0259621\PaperNo_2\All\csv_thanh_vui_02.csv"
    with open(file_name_csv, mode='w', newline='', encoding='utf-8') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(['Date', 'Max_Temp', 'Min_Temp', 'Stable_Temps'])
        ...
        for row in list_stable_days:
            ...
            writer.writerow([date, max_temp, min_temp, stable_temp])
        
            count += 1
            
        print(f"Write {count} rows into file csv successfully!")
...
def store_data_in_sqlite(data):
    ...
    file_name_database = r"D:\Study-AI\Python\Practical\9_PRP201c_SP25_PE1_0259621\PaperNo_2\All\weather_thanh_vui_02.db"
    with sqlite3.connect(file_name_database) as conn:
        cursor = conn.cursor()
        
        cursor.execute("""
                       DROP TABLE IF EXISTS weather_forecast_02
                       """)
        
        cursor.execute("""
                       CREATE TABLE IF NOT EXISTS weather_forecast_02(
        ...
        for index in range(len(dates)):
            ...
            cursor.execute("""
                        INSERT OR IGNORE INTO weather_forecast_02 (date, max_temp, min_temp, fluctuation)
                        VALUES (?, ?, ?, ?)
                        """, (date, max_temp, min_temp, fluctuation))
            
            count += 1
        print(f"Insert {count} rows into database successfully!")
        conn.commit()
...
def unstable_warm_days(data):
    ...
    with sqlite3.connect(file_name_database) as conn:
        ...
        cursor.execute("""
                    ...
                    FROM weather_forecast_02
                    WHERE min_temp >= 9
                       """)
        
        list_unstable_days = [row for row in cursor.fetchall()]
...
def generate_forecast_statistics(data):
    ...
    sum_max_temps = sum(temperature_2m_maxs)
    sum_min_temps = sum(temperature_2m_mins)
    
    avg_max_temps = sum_max_temps / count_days
    avg_min_temps = sum_min_temps / count_days
    number_of_stable_days = len(detect_stable_days(data))
    number_of_unstable_days = len(unstable_warm_days(data))
    full_temps = max(temperature_2m_maxs) - max(temperature_2m_mins)
    
    print(f"Average Of Max Temperature: {avg_max_temps:.2f}")
    print(f"Average Of Max Temperature: {avg_min_temps:.2f}")
    print(f"Number Of Stable Days: {number_of_stable_days}")
    print(f"Number Of Unstable Days: {number_of_unstable_days}")
    print(f"Full Range Of Temperature: {full_temps}")
    
# ======================================= Question 8: ===========================================
def plot_stable_and_unstable_days(data, list_stable_days, list_unstable_days):
    list_stable_days_date = [row[0] for row in list_stable_days]
    list_stable_days = [row[1] for row in list_stable_days]
    list_unstable_days_date = [row[0] for row in list_unstable_days]
    list_unstable_days = [row[1] for row in list_unstable_days]
    
    print(list_stable_days_date)
    print(list_unstable_days_date)
    
    print(list_stable_days)
    print(list_unstable_days)
    
    plt.plot(list_stable_days_date, list_stable_days, marker='o', label="Max Temperature")
    plt.plot(list_unstable_days_date, list_unstable_days, marker='o', label="Min Temperature")
    plt.title("Summary Max And Min Temperature")
    plt.xlabel("Date")
...
# --------------------------------------- Question 2: -------------------------------------------
    print(f"\n{'=' * 50} Question 2 {'=' * 50}")
    list_top3_warmest_days = find_top3_warmest_days(data)
    print("Top 3 Warmest Days:")
    for row in list_top3_warmest_days:
        print(f"Date: {row[0]} - Temperature: {row[1]}\u00b0C")
        
# --------------------------------------- Question 3: -------------------------------------------
    print(f"\n{'=' * 50} Question 3 {'=' * 50}")
    list_stable_days = detect_stable_days(data)
    print("List Stable Days:")
    for row in list_stable_days:
        print(f"Date: {row[0]} - Stable Temperature: {row[1]}\u00b0C")
...
# --------------------------------------- Question 6: -------------------------------------------
    ...
    print("List Unstable Days:")
    for row in list_unstable_days:
        print(f"Date: {row[0]} - Unstable Temperature: {row[1]}\u00b0C")
